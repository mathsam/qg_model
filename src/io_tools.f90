module io_tools                   !-*-f90-*-
  ...
  !
  ! Dependencies: par_tools (to call wrapper for MPI_Finalize from Message)
  !               and you need to know the default record width
  !               for your compiler/system.  Read #2 under Pass_params
  !               carefully.
  !
  ! ROUTINES:
  !
  ! Pass_params(datadirin,recunitin,my_pein,io_rootin):  Call to set internal 
  !   values of four parameters necessary for routines to work
  !   If this routine is not called, these two variables will default to
  !
  !   1. datadir:  sets the directory to which all data is written. DEFAULT: './'
  !
  ...
  !     recunit=2.  DEFAULT:  8
  !
  !   3. my_pe:  OPTIONAL.  Pass io_tools current processor id for MPI runs.
  !              DEFAULT:  0
  !
  !   4. io_root: OPTIONAL.  Pass io_tools the id of the processor that will 
  !      handle all io.  DEFAULT:  0
  !  
  ...
  !
  ! Write_field(field,fname,frame,zfirst):  Write 'frame' (an integer counter) of
  ...
  !     not present.  If integer zfirst is present (no matter what its value), 
  !     write_field assumes that first dimension is z dimension (for 3d arrays 
  !     only).  This is to accomodate new array ordering in MPI-based code.
  !
  ! Read_field(field,fname,frame,exclude_dd,zfirst,all_pes):  Same as for Write_field, only
  ...
  !    for 'fname' in 'datadir' (default). If integer all_pes is passed, the file
  !    will be read from all processors (default is to read only to io_root).
  !
  !**************************************************************************
  ...
  integer,save            :: io_root=0       ! pe used for i/o in mpi run
  integer,save            :: my_pe=0         ! current pe in mpi run
  character(80),save      :: datadir='./'
  character(8)            :: runlog='run.log'
  ...
  interface Pass_params
     module procedure Pass_params
  end interface
  interface Open_file
...
contains
  ...
  subroutine Pass_params(datadirin,recunitin,my_pein,io_rootin)
    character(*),intent(in)      :: datadirin
    integer,intent(in)            :: recunitin
    integer,intent(in),optional   :: my_pein,io_rootin
    datadir = datadirin
    recunit = recunitin
    my_pe = my_pein      ! current processor
    io_root = io_rootin ! pe that handles IO
  end subroutine Pass_params

  subroutine Message(msg,tag,fatal,r_tag)
    use par_tools, only: end_par
    ...
    if (present(fatal)) then   ! Call this from all pes
       call end_par
       stop
    endif
  ...
  subroutine Read_value(field,fname,frame,exclude_dd,all_pes)
    real                          :: field
    character(*),intent(in)       :: fname
    integer,optional,intent(in)   :: frame, exclude_dd,all_pes
    integer                       :: offset=0

    if (my_pe==io_root.or.present(all_pes)) then
  ...
  subroutine Read_array(field,fname,frame,exclude_dd,all_pes)
    real,dimension(:)             :: field
    character(*),intent(in)       :: fname
    integer,optional,intent(in)   :: frame, exclude_dd,all_pes
    integer                       :: n,j, offset=0
  ...
  subroutine Write_field2(field,fname,frame)
    ...
    if (my_pe==io_root) then
       ...
       do j=1,ny
          write(fida,rec=j+offset,iostat=iocheck) field(1:nx,j) 
  ...
  subroutine Read_field2(field,fname,frame,exclude_dd,all_pes)
    ...
    if (my_pe==io_root.or.present(all_pes)) then
       ...
       if (present(frame)) offset = ny*(frame-1)
       do j = 1,ny
          read(fida,rec=j+offset,iostat=iocheck) field(1:nx,j) 
  ...
  subroutine Write_field2c(field,fname,frame)
    ...
    if (my_pe==io_root) then
       ...
       do j=1,ny
          write(fida,rec=j+offsetr,iostat=iocheck) fr(1:nx,j)
          if (iocheck/=0) call Message('Write_field2c error, name & iocheck: '&
               &//fname,iocheck,'y')
          write(fida,rec=j+offseti,iostat=iocheck) fi(1:nx,j)
          if (iocheck/=0) call Message('Write_field2c error, name & iocheck: '&
               &//fname,iocheck,'y')
       enddo
       close(fida)
  ...
  subroutine Read_field2c(field,fname,frame,exclude_dd,all_pes)
    ...
    if (my_pe==io_root.or.present(all_pes)) then
       ...
       do j=1,ny
          read(fida,rec=j+offsetr,iostat=iocheck) fr(1:nx,j)
          if (iocheck/=0) call Message('Read_field2c error, name & iocheck: '&
               &//fname,iocheck,'y')
          read(fida,rec=j+offseti,iostat=iocheck) fi(1:nx,j)
          if (iocheck/=0) call Message('Read_field2c error, name & iocheck: '&
               &//fname,iocheck,'y')
       enddo
       field=cmplx(fr,fi)
  ...
  subroutine Write_field3(field,fname,frame,zfirst)
    real,dimension(:,:,:),intent(in)      :: field
    character(*),intent(in)               :: fname
    integer,optional,intent(in)           :: frame,zfirst
    integer                               :: nx,ny,nz,i,j,k,offset=0

    if (my_pe==io_root) then
       if (present(zfirst)) then  ! Assume first dim is z
          nx = size(field,2); ny = size(field,3); nz = size(field,1)
          call Open_file(fida,fname,'unknown',nx)
          ...
          enddo
          close(fida)
       else                 ! do it the old way - z last dim
  ...
  subroutine Read_field3(field,fname,frame,exclude_dd,zfirst,all_pes)
    real,dimension(:,:,:),intent(out)     :: field
    character(*),intent(in)               :: fname
    integer,optional,intent(in)           :: frame,exclude_dd,zfirst,all_pes
    ...
    if (my_pe==io_root.or.present(all_pes)) then
       if (present(zfirst)) then  ! Assume first dim is z
          ...
          do k=1,nz
             do j=1,ny
                read(fida,rec=(k-1)*ny+j+offset,iostat=iocheck) &
                     field(k,1:nx,j)
                if (iocheck/=0) call Message('Read_field3p error, name & iocheck: '&
                     &//fname,iocheck,'y')
             enddo
          enddo
          close(fida)
       else                      ! do it the old way - z is last dim
          nx = size(field,1); ny = size(field,2); nz = size(field,3)
          if (present(exclude_dd)) then
             call Open_file(fida,fname,'old',nx,exclude_dd)
          else
          ...
          do k=1,nz
             do j=1,ny
                read(fida,rec=(k-1)*ny+j+offset,iostat=iocheck) field(1:nx,j,k)
                if (iocheck/=0) call Message('Read_field3 error, name & iocheck: '&
                     &//fname,iocheck,'y')
             enddo
          enddo
          close(fida)
  ...
  subroutine Write_field3c(field,fname,frame,zfirst)
    complex,dimension(:,:,:),intent(in)   :: field
    character(*),intent(in)               :: fname
    integer,optional,intent(in)           :: frame, zfirst
  ...
  subroutine Read_field3c(field,fname,frame,exclude_dd,zfirst,all_pes)
    complex,dimension(:,:,:),intent(out)  :: field
