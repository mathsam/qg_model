module numerics_lib
  ...
  ! in Matlab.
  !
  ! Routines: march, ring_integral, Spline, Splint, Sortvec, Findzeros,
  !           sub2ind, Ran, Tridiag, Tridiag_cyc, Tri2mat, Diag, Diffz2
  !
  ...
  public :: Ring_integral, Spline, Splint, Sortvec, Findzeros, Sub2ind, &
       Ran, Diag, Tridiag, Tridiag_cyc, Tri2mat, March, March_implicit, Diffz2, Diffz
  ...
  interface March
     module procedure March1, March2, March3
  end interface
...
contains
  ...
  function March1(f_now,f_o,rhs,dt,rob,calls)  result(f)
    ...
    real,dimension(:),intent(in)                :: f_now,rhs
    real,dimension(:),intent(inout)             :: f_o
    real,dimension(size(f_now,1))               :: f
  ...
  function March_implicit(f_now,f_o,rhs,op,dt,rob,calls)  result(f)
    ...
    ! On first call, you need calls=0.
    !
    ! This version gets the future step implicitly, in conjunction with 
    ! diffusion (used now for vertical diffusion of tracer).
    !**************************************************************
    ...
    integer,intent(inout)                                        :: calls
    ! Local
    real,dimension(size(op,1),-1:1)                              :: op_t
    ...
    if (calls==0) then        ! Small Euler forward
       ...
       f = tridiag3(f_now + (.5*dt)*rhs,op_t)
       f_o = f_now
       calls = 1
    ...
    elseif (calls==1) then    ! Small leap-frog
       ...
       f = tridiag3(f_o + (2*dt)*rhs,op_t)
       f_o = (1.-2.*rob)*f_now + rob*(f_o + f)

    endif

  end function March_implicit
  ...
  function Ring_integral3(f,xv,yv,nf)  result(int)
    ...
    integer,dimension(:),intent(in)        :: xv, yv
    integer,intent(in)                     :: nf
    real,dimension(1:nf,size(f,1))         :: int
    ! local
    integer                                :: n, nx, ny, rad
    integer,dimension(size(f,2),size(f,3)) :: radius_arr
  ...
  function Ran1(idum,nx) result(ranf)
    ...
    do x = 1,nx    
       j=1+(irsize*iy)/m
       if ((j>irsize).or.(j<1)) stop 'j out of bounds in ran1'
       iy=ir(j)
       ranf(x)=iy*rm
       idum=mod(ia*idum+ic,m)
       ir(j)=idum
  ...
  function Ran2(idum,nx,ny) result(ranf)
...
!!$    call write_field(mat1,'mat1')
  ...
  function tridiag3(f_in,op) result(f_out)
    ...
    complex,dimension(:,:,:),intent(in)                       :: f_in
    real,dimension(:,-1:),intent(in)                          :: op
    complex,dimension(size(f_in,1),size(f_in,2),size(f_in,3)) :: f_out
    ! Local 
    complex,dimension(size(f_in,1)) :: gamma
    complex                         :: bet
    integer                         :: nv, n
  ...
  end function tridiag3

  !*********************************************************************

  function diffz(f_in,dz) result(f_out)

    !*********************************************************************
    ! Vertical first derivative.  Assumes layer interfaces given by
    ! values in array dz, which has same length as dimension 1 of f_in.
    ! Calculates centered difference.  Boundaries use one-sided derivs.
    !*********************************************************************
    ...
    ! Local
    real,dimension(size(f_in,1)-2)                            :: idz
    integer                                                   :: nz

    nz = size(dz)

    if (size(f_in,1)/=nz) call Message('Error:diffz: size dz neq dim 1 of f_in',fatal='y')

    idz = 1/( 0.5*(dz(1:nz-2)+dz(3:nz)) + dz(1:nz-2))

    f_out(2:nz-1,:,:) =  idz*( f_in(1:nz-2,:,:) - f_in(3:nz,:,:) )
    f_out(1 ,:,:) = 2*( f_in(1,:,:) - f_in(2,:,:) )/(dz(1)+dz(2))
    f_out(nz,:,:) = 2*( f_in(nz-1,:,:) - f_in(nz,:,:) )/(dz(nz-1)+dz(nz))

  end function diffz

  !*********************************************************************

  function diffz2(f_in,dz) result(f_out)

    !*********************************************************************
    ! Vertical second derivative.  Assumes layer interfaces given by
    ! values in array dz, which has same length as dimension 1 of f_in.
    ! Calculates centered difference.  Boundary values set to have
    ! same second derivative as inside neighbors.
    !*********************************************************************
    ...
    ! Local
    real,dimension(size(f_in,1)-2)                            :: idz2up, idz2dn
    integer                                                   :: nz

    nz = size(dz)

    if (size(f_in,1)/=nz) call Message('Error:diff2z: size dz neq dim 1 of f_in',fatal='y')

    idz2up = 2/( dz(2:nz-1) * (dz(1:nz-2) + dz(2:nz-1)) )
    idz2dn = 2/( dz(2:nz-1) * (dz(2:nz-1) + dz(3:nz  )) )

    f_out(2:nz-1,:,:) =   idz2up*( f_in(1:nz-2,:,:) - f_in(2:nz-1,:,:) ) &
                        - idz2dn*( f_in(2:nz-1,:,:) - f_in(3:nz  ,:,:) )
