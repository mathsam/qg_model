module strat_tools        
  ...
  !
  ! Routines: Strat_params, Make_strat, Get_z, 
...
contains
  ...
  !*************************************************************************
   ...
   function Strat_params(dz,drho,F,Fe,surface_bc) result(op)
    ...
    real,dimension(1:size(dz)-1),intent(in)        :: drho
    real,intent(in)                                :: F, Fe
    character(*),intent(in)                        :: surface_bc
    real,dimension(1:size(dz),-1:1)                :: op
    integer                                        :: nz
  ...
  subroutine Get_vmodes(dz,drho,F,Fe,kz,vmode,surface_bc)
    ...
    if (nz>2) then
       ...
       op=0.; opv=0.; vmodeout=0.; ind_map=0

       select case (trim(surface_bc))
       case ('periodic');     opv = strat_params(dz,drho,F,Fe,surface_bc)
       case default;          opv = strat_params(dz,drho,F,Fe,'rigid_lid')
       end select
  ...
  function Get_tripint(dz,rho,surface_bc,hfi,drt,drb) result(tripint)
    ...
    elseif (nz==2) then
       ...
       tripint(2,1,1) = 0.; tripint(1,2,1) = 0.; tripint(1,1,2) =0.   ! added by eobrien
       tripint(2,2,1) = 1.
  ...
  function Layer2mode(f,vmode,dz,maxmode) result(fm)
    ...
    real,dimension(size(f,1),size(f,1)),intent(in)   :: vmode
    real,dimension(size(f,1)),intent(in)             :: dz
    integer,intent(in),optional                      :: maxmode
    integer                                          :: ix,iy,n,nx,ny,nz,mmax

    nx = size(f,2); ny = size(f,3); nz = size(f,1)
    mmax = nz
    ...
    do iy = 1,ny
       do ix = 1,nx
          do n = 1,mmax
             fm(n,ix,iy) = dot_product((vmode(:,n)*dz),f(:,ix,iy))
          enddo
       enddo
    enddo
  ...
  function Mode2layer(fm,vmode,maxmode) result(f)
    ...
    complex,dimension(size(vmode,1),size(fm,2),size(fm,3)) :: f
    integer,intent(in),optional                            :: maxmode
    integer                                                :: ix,iy,nx,ny,nz,nzt,mmax

    nx = size(fm,2); ny = size(fm,3); nzt = size(vmode,1); nz = size(vmode,2)
    mmax = nz
    ...
    do iy = 1,ny             ! Project modes onto layers
       do ix = 1,nx
          f(:,ix,iy) = matmul(vmode(:,1:mmax),fm(1:mmax,ix,iy))
       enddo
    enddo
